...
> reset && zip -r stage2.zip . -x "**/node_modules/**" -x "target/**" -x ".git/**" -x ".idea/**" -x ".vscode/**"
...
```
docker run -p 8080:8080 library:latest
```
...
If the pods say `Container image "library:latest" is not present with pull policy of Never` then you have not built your application using Docker or there is a mismatch in the name. Use `docker images ls` to see which images are available.
