...
//  Класс для доступа к данным по http
// Каждая сущьность книги, автор итп имеют собственный экземпляр
export class DataProviderImpl {
    ...
    public async getMany(params: GetManyParams): Promise<GetManyResult<RaRecord>> {
        const msg = ` [${uniqueId()}] ${DataProviderImpl.name} ${this.getMany.name}`
        this.logger.debug('REQUEST' + msg, params);
        
        try {
            const response = await this.http.get(`/${this.resource}/search/findByIdIn?ids=${encodeURI(params.ids.join(','))}`);
            this.logger.debug('RESPONSE' + msg, response);
            
            return { data: response.data._embedded[this.resource] };
        } catch (e) {
            this.logger.error('ERROR' + msg, e);
            throw e;
        }
